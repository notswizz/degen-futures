...
import { getBuyCost } from '../../../lib/bondingCurve';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('Buy API - User ID:', userId);
    ...
    if (!userDoc) {
      console.log('User not found in Buy API');
      return res.status(404).json({ message: 'User not found' });
    }
    
    console.log('User found in Buy API. Balance:', userDoc.balance);
    ...
    console.log('New token payload after buy:', payload);
    ...
    console.error('Error in buy API:', err);
